...
VCS_F_UNTRACKED(){
	...
	if [[ "$VCS_UNTRACKED" > 0 ]]; then
		echo "${VCS_UNCOMMITED}${VCS_S_UNTRACKED}${VCS_UNTRACKED}" | xargs | cat - <(echo " ") | tr -d '\n';
	fi
}
...
VCS_F_UNSTAGED(){
	...
	if [[ "$VCS_UNSTAGED" > 0 ]]; then
		echo "${modified}${VCS_S_UNSTAGED}${VCS_UNSTAGED}" | xargs | cat - <(echo " ") | tr -d '\n';
	fi
}
...
VCS_F_UNCOMMITED(){
	...
	if [[ "$VCS_UNCOMMITED" > 0 ]]; then
		echo "${modified}${VCS_S_UNCOMMITED}${VCS_UNCOMMITED}" | xargs | cat - <(echo " ") | tr -d '\n';
	fi
}
...
VCS_F_BEHIND(){
	...
	if [[ "$VCS_BEHIND" > 0 ]]; then
		echo "${clean}${VCS_S_BEHIND}${VCS_BEHIND}" | xargs | cat - <(echo " ") | tr -d '\n';
	fi
}
...
VCS_F_AHEAD(){
	...
	VCS_S_AHEAD="⇡"
	if [[ "$VCS_AHEAD" > 0 ]]; then
		echo "${clean}${VCS_S_AHEAD}${VCS_AHEAD}" | xargs | cat - <(echo " ") | tr -d '\n';
	fi
}
